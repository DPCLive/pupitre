...
const ConnectionState = {
	closed: {id:1, description: 'Not connected'},
	attempting: {id:2, description: 'Attempting to connect'},
	establishing: {id: 3, description: 'Establishing connection'},
	open: {id:4, description: 'connected'}
};
...
function Device(atemIpAddress){
	...
	function AtemPacket(msg){
		...
		if (this.isConnect()) {
			...
			while (cursor < body.length-1) {
				...
				if (cmd.name == 'Time') {
					currentTime = [cmd.data[0], cmd.data[1], cmd.data[2], cmd.data[3]]
					lastTime = currentTime
				}
				cmd.data.timecode = currentTime || lastTime
	 ...
	 */
	this.setProgram = function(sourceID, mE = 0) {
		var data = new Buffer(4);
		data.writeUInt32BE(sourceID, mE);
	 ...
	 * @param {SourceID} sourceID the ID of the source
	 */
	this.setPreview = function(source, mE = 0) {
		var data = new Buffer(4);
		data.writeUInt32BE(source, mE);
	...
	this.on('PrgI', function(d) {
		const source = d.readUInt16BE(2),
			mE = parseString(d[0]);
		//const videoSource = parseString(d.slice(2,4));
		/**
		 ...
		 */
		atem.emit('programBus', mE, source);
		atem.emit('programBusWithTime', {source: source, timecode: d.timecode});
		// todo inspect the other bytes in PrgI
	});

	// Position des transitions get
	this.on('TrPs', function(d) {
		const source = d.readUInt16BE(4),
			mE = parseString(d[0]);
		 ...
		 * @property {SourceID} source The new program source
		 */
		atem.emit('transitionPosition', mE, source);
	});
	
	// get Keyer
	this.on('TrSS', function(d) {
		const source = d.readUInt16BE(1);
		//const mE = d.readUInt16BE(0);
		const mE = parseString(d[0]);
		const style = parseString(d[1]);
		const NextTransition = parseString(d[2]);
		const styleNext = parseString(d[3]);
		const NextTransitionNext = parseString(d[4]);
		 ...
		 * @property {SourceID} source The new program source
		 */
		atem.emit('getKeyer', NextTransitionNext, source);
	});
	...
	this.on('KeOn', function(d) {
		...
		//const mE = d.readUInt16BE(0);
		const mE = parseString(d[0]);
		const keyer = parseString(d[1]);
		const Enabled = parseString(d[2]);
		 ...
		 * @property {SourceID} source The new program source
		 */
		atem.emit('getKeyOn', mE, keyer, Enabled, source);
	});

	//get Keyer Base
	this.on('KeBP', function(d) {
		//const source = d.readUInt16BE(1);
		//const mE = d.readUInt16BE(0);
		const mE = parseString(d[0]);
		const keyer = parseString(d[1]);
		const Enabled = parseString(d[3]);
		const Fill = parseString(d.readUInt16BE(6),d.readUInt16BE(7));
		const Key = parseString(d.readUInt16BE(8));
		/**
		 * When the selected program bus changes this event will be fired.
		 * @event Device#programBus
		 * @property {SourceID} source The new program source
		 */
		atem.emit('getKeyBP', mE, keyer, Fill, Key);
	});
	
	this.on('MPCE', function(d) {
		const MP = parseString(d[0]);
		const Type = parseString(d[1]);
		const Still = parseString(d[2]);
		atem.emit('getMPCE', MP, Type, Still);
		});
		
		this.on('MPfe', function(d) {
		const MP = parseString(d[0]);
		const Type = parseString(d[1]);
		const FileNameLength = d[23];
		const FileName = parseString(d.slice(24, 24 + FileNameLength));
		atem.emit('getMPfe', FileName, FileNameLength);
		});
